...
import { onboardHouse } from "../../services/houseService";
...
export default function OnboardingHouse() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.images.length < 1) {
      toast.error("Please upload at least 1 property image.");
      return;
    }
    ...
    try {
      ...
      await onboardHouse(data);
      toast.success("House onboarded successfully!");
      ...
      toast.error(error.response?.data?.error || "Failed to onboard house");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <PageBreadcrumb pageTitle="Onboarding Houses" />
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-xl dark:border-slate-800 dark:bg-slate-900 md:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {formData.images.length > 0 && (
                  <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {formData.images.map((file, idx) => (
                      <div key={idx} className="relative group rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700 aspect-[4/3] bg-slate-100 dark:bg-slate-800">
                        <img 
            ...
            <div className="pt-8 mt-8 border-t border-slate-200 dark:border-slate-800 flex justify-end">
              ...
              >
                {isLoading ? "Onboarding..." : "Onboard Property"}
              </button>
            </div>
